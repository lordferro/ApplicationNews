...
function onNextMonth() {
  ...
  dataSelected.textContent = `${addLeadingZero(formData.day)}/${addLeadingZero(
    months.indexOf(month.textContent) + 1
  )}/${year.textContent}`;
...
function onDateSelection(event) {
  ...
  dataSelected.textContent = `${addLeadingZero(
    event.target.textContent
  )}/${addLeadingZero(months.indexOf(month.textContent) + 1)}/${
    year.textContent
  }`;
...
async function mainAsync() {
  const data = new Date(
    ...
    months.indexOf(month.textContent)
  );
  ...
  mayDates.map(e => {
    if (
      e.iso ===
      `${dates.getFullYear()}-${addLeadingZero(
        dates.getMonth() + 1
      )}-${addLeadingZero(dates.getDate())}`
    ) {
      e.type = 'today';
    }
  });
...
dateField.addEventListener('click', onOpenCalendar);

function onOpenCalendar() {
  dateField.removeEventListener('click', onOpenCalendar);
  ...
  body.addEventListener('click', onOutOfCalendarClick);
  dateField.addEventListener('click', onCloseCalendar);
}

function onCloseCalendar() {
  dateField.removeEventListener('click', onCloseCalendar);
...
function onOutOfCalendarClick(evt) {
  if (!evt.target.closest('.calendar-box')) {
    onCloseCalendar();
    body.removeEventListener('click', onOutOfCalendarClick);
  } 
}
